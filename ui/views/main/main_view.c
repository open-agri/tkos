...
static void refresh_cb(lv_obj_t *obj, lv_event_t event) {
  ...
  // Left arc's value label
    ...
    ESP_LOGV(TAG, "Received a refresh event for left value label, value is "
                  "<TODO> [km/h - MPH].");
    // lv_obj_realign(arc_r_big_label);
  }
...
tk_view_t build_main_view() {
  ...
  tk_bottom_bar_button_t left_bar_button = {
      .text = "Brightness", .click_callback = left_button_click_callback};
