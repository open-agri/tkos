...
LV_EVENT_CB_DECLARE(item_control_event_cb) {
  ...
  if (e == LV_EVENT_FOCUSED) {
    ...
    if (item->type == TK_MENU_ITEM_SWITCH &&
        lv_group_get_editing(menu->group)) {
  ...
  if (e == LV_EVENT_VALUE_CHANGED) {
    ...
    case TK_MENU_ITEM_SLIDER:
      ...
      double out =
          perc * (item->binding_max - item->binding_min) + item->binding_min;
    ...
    default:
      ESP_LOGW(TAG,
               "Trying to update %s binding, but its type is not implemented.",
               item->desc);
      break;
    }
    lock_refresh = false;
  ...
  if (e == LV_EVENT_REFRESH && !lock_refresh) {
    ...
    switch (item->type) {
    case TK_MENU_ITEM_SWITCH:
      *(bool *)item->binding ? lv_switch_on(item->control, LV_ANIM_ON)
                             : lv_switch_off(item->control, LV_ANIM_ON);
      break;
    case TK_MENU_ITEM_SLIDER:
      ...
      lv_slider_set_value(
          item->control, val,
          LV_ANIM_OFF); // FIXME: Does the exact opposite, animations needed
                        // otherwise slider gets stuck.

      break;
    default:
      ESP_LOGW(TAG,
               "Trying to refresh %s control, but its type is not implemented.",
               item->desc);
      break;
    }
  }
...
lv_obj_t *tk_menu_create(lv_obj_t *parent, lv_group_t *group, tk_menu_t *menu) {
  ...
  lv_obj_add_style(menu_widget, LV_PAGE_PART_SCROLLABLE,
                   &tk_style_far_background);
  lv_obj_set_style_local_pad_inner(menu_widget, LV_PAGE_PART_SCROLLABLE,
                                   LV_STATE_DEFAULT, 0);
  ...
  for (int i = 0; i < menu->items_count; i++) {
    ...
    lv_obj_add_style(current_item->base, LV_CONT_PART_MAIN,
                     &tk_style_menu_button);
    ...
    case TK_MENU_ITEM_SWITCH:
      ...
      if (current_item->binding != NULL) {
        *(int *)current_item->binding
            ? lv_switch_on(current_item->control, false)
            : lv_switch_off(current_item->control, false);
      }
      break;
    ...
    case TK_MENU_ITEM_SLIDER:
      ...
      lv_obj_add_style(current_item->control, LV_SLIDER_PART_KNOB,
                       &tk_style_invisible_when_disabled);
    ...
    if (current_item->control != NULL)
      lv_obj_add_style(current_item->control, LV_OBJ_PART_MAIN,
                       &tk_style_no_outline);

    // Call value change
    if (current_item->value_change_cb != NULL)
      (current_item->value_change_cb)(current_item);
  }
