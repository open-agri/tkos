...
static void left_button_event_callback(lv_obj_t *obj, lv_event_t event) {
  ...
  switch (event) {
	case LV_EVENT_CLICKED: view_navigate_back();
	  break;
  }
}
...
lv_obj_t *build_bottom_bar(tk_bottom_bar_configuration configuration) {
  ...
  if (configuration.right_button.text != NULL) {
	...
	lv_obj_set_style_local_bg_color(right_button, LV_BTN_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
	lv_obj_set_style_local_bg_color(right_button, LV_BTN_PART_MAIN, LV_STATE_PRESSED, LV_THEME_DEFAULT_COLOR_PRIMARY);
	lv_obj_set_style_local_radius(right_button, LV_BTN_PART_MAIN, LV_STATE_DEFAULT, 0);
	lv_obj_set_style_local_border_width(right_button, LV_BTN_PART_MAIN, LV_STATE_DEFAULT, 0);
	lv_obj_set_style_local_outline_width(right_button, LV_BTN_PART_MAIN, LV_STATE_DEFAULT, 0);
